...
- **Churn driver table** — features with positive mean SHAP values only, actively pushing customers toward churn, with impact strength and business insight for
  each driver
...
| `TotalCharges` column shows errors | Ensure TotalCharges contains numeric values; whitespace entries are handled automatically but fully non-numeric columns 
